...
NSString * Carter_Jacob_Project_2$ModuleRequireFormat = @"(function(exports){"
...
@implementation Carter_Jacob_Project_2Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_carter_jacob_project_2);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Carter_Jacob_Project_2 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Carter_Jacob_Project_2" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_carter_jacob_project_2 = [[Carter_Jacob_Project_2Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_carter_jacob_project_2];
	
	NSString *_carter_jacob_project_2NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _carter_jacob_project_2NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_carter_jacob_project_2 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Carter_Jacob_Project_2$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.carter_jacob_project_2.kroll" 
