...
NSString * k7_4_Code_Refactoring$ModuleRequireFormat = @"(function(exports){"
...
@implementation k7_4_Code_RefactoringObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_k7_4_code_refactoring);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create k7_4_Code_Refactoring global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "k7_4_Code_Refactoring" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_k7_4_code_refactoring = [[k7_4_Code_RefactoringObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_k7_4_code_refactoring];
	
	NSString *_k7_4_code_refactoringNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _k7_4_code_refactoringNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_k7_4_code_refactoring valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:k7_4_Code_Refactoring$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.k7_4_code_refactoring.kroll" 
