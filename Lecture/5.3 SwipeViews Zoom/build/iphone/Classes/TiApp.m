...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kk5_3_SwipeViews_ZoomUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kk5_3_SwipeViews_ZoomUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a k5_3_SwipeViews_Zoom app using the node.js CLI
