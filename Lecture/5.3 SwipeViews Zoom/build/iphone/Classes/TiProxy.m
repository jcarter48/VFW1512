...
//Rare exceptions to indicate a bug in the _k5_3_swipeviews_zoom code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_k5_3_swipeviews_zoom = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_k5_3_swipeviews_zoom]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.k5_3_swipeviews_zoom.module"
