...
NSString * k5_3_SwipeViews_Zoom$ModuleRequireFormat = @"(function(exports){"
...
@implementation k5_3_SwipeViews_ZoomObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_k5_3_swipeviews_zoom);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create k5_3_SwipeViews_Zoom global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "k5_3_SwipeViews_Zoom" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_k5_3_swipeviews_zoom = [[k5_3_SwipeViews_ZoomObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_k5_3_swipeviews_zoom];
	
	NSString *_k5_3_swipeviews_zoomNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _k5_3_swipeviews_zoomNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_k5_3_swipeviews_zoom valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:k5_3_SwipeViews_Zoom$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.k5_3_swipeviews_zoom.kroll" 
