...
NSString * k5_1_SwipeViews$ModuleRequireFormat = @"(function(exports){"
...
@implementation k5_1_SwipeViewsObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_k5_1_swipeviews);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create k5_1_SwipeViews global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "k5_1_SwipeViews" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_k5_1_swipeviews = [[k5_1_SwipeViewsObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_k5_1_swipeviews];
	
	NSString *_k5_1_swipeviewsNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _k5_1_swipeviewsNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_k5_1_swipeviews valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:k5_1_SwipeViews$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.k5_1_swipeviews.kroll" 
