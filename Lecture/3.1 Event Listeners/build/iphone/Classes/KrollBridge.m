...
NSString * k3_1_Event_Listeners$ModuleRequireFormat = @"(function(exports){"
...
@implementation k3_1_Event_ListenersObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_k3_1_event_listeners);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create k3_1_Event_Listeners global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "k3_1_Event_Listeners" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_k3_1_event_listeners = [[k3_1_Event_ListenersObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_k3_1_event_listeners];
	
	NSString *_k3_1_event_listenersNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _k3_1_event_listenersNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_k3_1_event_listeners valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:k3_1_Event_Listeners$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.k3_1_event_listeners.kroll" 
