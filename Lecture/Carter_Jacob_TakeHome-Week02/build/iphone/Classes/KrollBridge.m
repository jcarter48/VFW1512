...
NSString * Carter_Jacob_Take_Home_Activity1$ModuleRequireFormat = @"(function(exports){"
...
@implementation Carter_Jacob_Take_Home_Activity1Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_carter_jacob_take_home_activity1);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_carter_jacob_take_home_activity1 gc];
}
...
{
	// create Carter_Jacob_Take_Home_Activity1 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Carter_Jacob_Take_Home_Activity1" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_carter_jacob_take_home_activity1 = [[Carter_Jacob_Take_Home_Activity1Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_carter_jacob_take_home_activity1];
	
	NSString *_carter_jacob_take_home_activity1NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _carter_jacob_take_home_activity1NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_carter_jacob_take_home_activity1 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Carter_Jacob_Take_Home_Activity1$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.carter_jacob_take_home_activity1.kroll" 
