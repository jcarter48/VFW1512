...
NSString * k4_2_Layout_Modes$ModuleRequireFormat = @"(function(exports){"
...
@implementation k4_2_Layout_ModesObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_k4_2_layout_modes);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create k4_2_Layout_Modes global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "k4_2_Layout_Modes" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_k4_2_layout_modes = [[k4_2_Layout_ModesObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_k4_2_layout_modes];
	
	NSString *_k4_2_layout_modesNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _k4_2_layout_modesNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_k4_2_layout_modes valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:k4_2_Layout_Modes$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.k4_2_layout_modes.kroll" 
