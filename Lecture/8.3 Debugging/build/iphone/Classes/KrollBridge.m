...
NSString * k8_3_Debugging$ModuleRequireFormat = @"(function(exports){"
...
@implementation k8_3_DebuggingObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_k8_3_debugging);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create k8_3_Debugging global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "k8_3_Debugging" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_k8_3_debugging = [[k8_3_DebuggingObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_k8_3_debugging];
	
	NSString *_k8_3_debuggingNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _k8_3_debuggingNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_k8_3_debugging valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:k8_3_Debugging$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.k8_3_debugging.kroll" 
