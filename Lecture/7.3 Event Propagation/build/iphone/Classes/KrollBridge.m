...
NSString * k7_3_Event_Propagation$ModuleRequireFormat = @"(function(exports){"
...
@implementation k7_3_Event_PropagationObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_k7_3_event_propagation);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create k7_3_Event_Propagation global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "k7_3_Event_Propagation" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_k7_3_event_propagation = [[k7_3_Event_PropagationObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_k7_3_event_propagation];
	
	NSString *_k7_3_event_propagationNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _k7_3_event_propagationNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_k7_3_event_propagation valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:k7_3_Event_Propagation$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.k7_3_event_propagation.kroll" 
