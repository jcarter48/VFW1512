...
NSString * k6_3_TableViewRows$ModuleRequireFormat = @"(function(exports){"
...
@implementation k6_3_TableViewRowsObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_k6_3_tableviewrows);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create k6_3_TableViewRows global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "k6_3_TableViewRows" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_k6_3_tableviewrows = [[k6_3_TableViewRowsObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_k6_3_tableviewrows];
	
	NSString *_k6_3_tableviewrowsNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _k6_3_tableviewrowsNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_k6_3_tableviewrows valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:k6_3_TableViewRows$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.k6_3_tableviewrows.kroll" 
