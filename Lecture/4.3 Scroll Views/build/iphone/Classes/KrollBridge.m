...
NSString * k4_3_Scroll_Views$ModuleRequireFormat = @"(function(exports){"
...
@implementation k4_3_Scroll_ViewsObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_k4_3_scroll_views);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create k4_3_Scroll_Views global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "k4_3_Scroll_Views" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_k4_3_scroll_views = [[k4_3_Scroll_ViewsObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_k4_3_scroll_views];
	
	NSString *_k4_3_scroll_viewsNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _k4_3_scroll_viewsNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_k4_3_scroll_views valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:k4_3_Scroll_Views$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.k4_3_scroll_views.kroll" 
