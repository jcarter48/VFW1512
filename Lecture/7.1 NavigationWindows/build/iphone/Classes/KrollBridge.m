...
NSString * k7_1_NavigationWindows$ModuleRequireFormat = @"(function(exports){"
...
@implementation k7_1_NavigationWindowsObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_k7_1_navigationwindows);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create k7_1_NavigationWindows global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "k7_1_NavigationWindows" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_k7_1_navigationwindows = [[k7_1_NavigationWindowsObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_k7_1_navigationwindows];
	
	NSString *_k7_1_navigationwindowsNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _k7_1_navigationwindowsNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_k7_1_navigationwindows valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:k7_1_NavigationWindows$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.k7_1_navigationwindows.kroll" 
