...
NSString * k6_4_Interactive_Tables$ModuleRequireFormat = @"(function(exports){"
...
@implementation k6_4_Interactive_TablesObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_k6_4_interactive_tables);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create k6_4_Interactive_Tables global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "k6_4_Interactive_Tables" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_k6_4_interactive_tables = [[k6_4_Interactive_TablesObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_k6_4_interactive_tables];
	
	NSString *_k6_4_interactive_tablesNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _k6_4_interactive_tablesNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_k6_4_interactive_tables valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:k6_4_Interactive_Tables$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.k6_4_interactive_tables.kroll" 
