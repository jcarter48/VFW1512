...
NSString * k6_1_Table_Views$ModuleRequireFormat = @"(function(exports){"
...
@implementation k6_1_Table_ViewsObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_k6_1_table_views);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create k6_1_Table_Views global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "k6_1_Table_Views" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_k6_1_table_views = [[k6_1_Table_ViewsObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_k6_1_table_views];
	
	NSString *_k6_1_table_viewsNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _k6_1_table_viewsNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_k6_1_table_views valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:k6_1_Table_Views$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.k6_1_table_views.kroll" 
