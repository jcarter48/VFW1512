...
NSString * k8_1_Interface_Demo$ModuleRequireFormat = @"(function(exports){"
...
@implementation k8_1_Interface_DemoObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_k8_1_interface_demo);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create k8_1_Interface_Demo global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "k8_1_Interface_Demo" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_k8_1_interface_demo = [[k8_1_Interface_DemoObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_k8_1_interface_demo];
	
	NSString *_k8_1_interface_demoNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _k8_1_interface_demoNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_k8_1_interface_demo valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:k8_1_Interface_Demo$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.k8_1_interface_demo.kroll" 
