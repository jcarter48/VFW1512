...
NSString * k4_1_ImageViews$ModuleRequireFormat = @"(function(exports){"
...
@implementation k4_1_ImageViewsObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_k4_1_imageviews);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create k4_1_ImageViews global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "k4_1_ImageViews" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_k4_1_imageviews = [[k4_1_ImageViewsObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_k4_1_imageviews];
	
	NSString *_k4_1_imageviewsNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _k4_1_imageviewsNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_k4_1_imageviews valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:k4_1_ImageViews$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.k4_1_imageviews.kroll" 
